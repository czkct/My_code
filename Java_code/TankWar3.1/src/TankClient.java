...
public class TankClient extends JFrame{
	...
	public void paint(Graphics g) { 
		...
		gOffScreen.drawString("战场子弹个数： "+missiles.size(), 10, 50);		  //炮弹个数
		gOffScreen.drawString("爆炸个数： "+booms.size(), 10, 70);            //爆炸个数
		gOffScreen.drawString("敌方坦克还剩： "+tanks.size(), 10, 90);         //坦克个数
		gOffScreen.drawString("坦克生命值： "+myTank.getLife(), 10, 110);     //坦克生命值剩余
		...
		if(tanks.size()<=0) {											   //如果敌人已经死光了   就加入一批新的敌人
		...
		myTank.collidesWithWall(w1);			    //这两句写不写可以决定我可不可以作弊！！！（我的坦克到底可不可以穿墙）
